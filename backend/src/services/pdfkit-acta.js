...
function dibujarActa(doc, o) {
  ...
  textoCampo(doc, M + 6, yy, 'Cédula', cli.doc || '', 42, wMid - 12); yy += 12;
  textoCampo(doc, M + 6, yy, 'Celular', cli.tel || '', 42, wMid - 12); yy += 12;
  textoCampo(doc, M + 6, yy, 'Email', cli.correo || '', 42, wMid - 12);
  yy = y + 20;
  textoCampo(doc, M + wMid + 6, yy, 'Marca/Modelo', v.model || '', 62, wMid - 12); yy += 12;
  textoCampo(doc, M + wMid + 6, yy, 'Placa', v.plate || '', 62, wMid - 12); yy += 12;
  textoCampo(doc, M + wMid + 6, yy, 'Tipo/Color', (r.tipoVeh || v.tipoVeh || '') + ' · ' + (r.color || v.color || ''), 62, wMid - 12); yy += 12;
  textoCampo(doc, M + wMid + 6, yy, 'Km', v.km || r.km || '', 62, wMid - 12);
  y += hCV;
  ...
  const acc = (r.accesorios || []).concat(r.documentos || []);
  let ax = M + 6, ay = y + 20, col = 0;
  acc.slice(0, 8).forEach((a) => {
    doc.font('Helvetica').fontSize(7.5).fillColor('#111').text('☑ ' + a, ax + (col % 2) * (wIzq / 2), ay, { width: wIzq / 2 - 8 });
    col++; if (col % 2 === 0) ay += 10;
  });
  caja(doc, M + wIzq, y, wDer, hAcc, 'Combustible');
  doc.font('Helvetica-Bold').fontSize(14).fillColor('#111').text(r.combustible || '½', M + wIzq, y + 18, { width: wDer, align: 'center' });
  doc.font('Helvetica').fontSize(7).fillColor('#666').text('Prioridad: ' + (r.prioridad || 'Media'), M + wIzq + 6, y + 40);
  if (r.bateria) doc.font('Helvetica').fontSize(6.5).fillColor('#666').text('Batería: ' + (r.bateriaMarca || '') + ' ' + (r.bateriaAmperaje ? r.bateriaAmperaje + 'A' : ''), M + wIzq + 6, y + 50, { width: wDer - 10 });
  y += hAcc;
  ...
  if (lados.length) {
    ...
    doc.fillColor('#fff').font('Helvetica-Bold').fontSize(8).text('Inspección visual — vistas registradas', M + 5, y + 3);
    doc.fillColor('#111');
    y += hTit;
    ...
    lados.slice(0, 6).forEach((lado, i) => {
      ...
      doc.font('Helvetica-Bold').fontSize(6.5).fillColor('#666').text(lado, cx, cy + 3, { width: wCelda, align: 'center' });
  ...
  if (damages.length) {
    doc.font('Helvetica-Bold').fontSize(8).fillColor('#111').text('Daños registrados (' + damages.length + '):', M, y + 4);
    y += 14;
    damages.slice(0, 6).forEach((d, i) => {
      const col = i % 2, fila = Math.floor(i / 2);
      const sev = (d.sev || 'leve').toLowerCase();
      doc.circle(M + col * 280 + 3, y + fila * 10 + 3, 3).fill(SEV_COLOR[sev] || '#2563eb');
      doc.fillColor('#111').font('Helvetica').fontSize(7.5).text(`#${i + 1} ${d.tipo || ''} — ${d.lado || ''} (${d.sev || ''})`, M + col * 280 + 10, y + fila * 10);
    });
  ...
  servicios.slice(0, 3).forEach((s) => {
    doc.font('Helvetica').fontSize(8).fillColor('#111').text(s.desc || '', M + 6, sy, { width: wMid - 70 });
    doc.text(s.precio ? mon + ' ' + Number(s.precio).toLocaleString('es-VE') : '', M + wMid - 60, sy, { width: 54, align: 'right' });
    sy += 12;
  });
  caja(doc, M + wMid, y, wMid, hServ, 'Autorización');
  doc.font('Helvetica').fontSize(6.5).fillColor('#333').text('Estoy de acuerdo con las condiciones de servicio y autorizo la reparación con el material necesario.', M + wMid + 6, y + 18, { width: wMid - 12 });
  ...
  if (r.obs && r.obs !== '—') {
    ...
    doc.font('Helvetica').fontSize(7.5).text(' ' + r.obs, { width: PAGE_W - 2 * M - 12 });
    y += hObs;
  }

  // --- Condiciones del servicio ---
  const cond = taller.condiciones || '1) Si el vehículo no se recoge una vez terminado el trabajo, se cobrará resguardo por día. 2) Es necesario liquidar el 100% del servicio para entregar la unidad.';
  const hCond = 50;
  ...
  doc.font('Helvetica').fontSize(6.5).fillColor('#444').text(cond, M + 6, y + 15, { width: PAGE_W - 2 * M - 12, height: hCond - 20, ellipsis: true });
}
...
function dibujarBitacora(doc, avances, orden, veh) {
  ...
  for (let i = 0; i < conFoto.length; i += 2) {
    ...
    grupo.forEach((a, idx) => {
      ...
      const buf = bufferDeBase64(a.foto);
      const fotoY = y + 34, fotoH = hCelda - 42;
      if (buf) {

...
export async function compartirActaPDF(tallerId, veh, tipo = 'acta') {
  ...
  try {
    ...
    // 2) Convertir a PDF (el propio teléfono lo arma — es el motor que siempre funcionó bien)
    let uri;
    try {
      const out = await Print.printToFileAsync({ html, base64: false, width: 595, height: 842 }); // A4 en puntos
      uri = out && out.uri;
    ...
    } catch (e) { /* si falla el renombrado, se comparte igual con el nombre original */ }
